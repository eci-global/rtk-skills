# CLAUDE.md — rtk-skills (ECI distribution toolkit)

Guidance for Claude Code working **in this repo**. Cursor/Codex/generic agents read the parallel [`AGENTS.md`](AGENTS.md). Pinned RTK version: **0.43.0** (see [`RTK_VERSION`](RTK_VERSION)).
...
rtk init --global                         # Claude Code  ·  rtk init -g --agent cursor for Cursor
```

- With the hook installed, run commands normally — output is rewritten transparently.
- Claude Code's built-in Read/Grep/Glob and MCP outputs **bypass** RTK (hook covers shell/Bash only); prefer shell `rg`/`cat`/`find` or `rtk read`/`rtk grep`/`rtk find` so output is filtered and counted toward savings.
