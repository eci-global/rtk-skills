...
For day-2 work — savings reports, troubleshooting, writing custom filters for niche toolchains (e.g., BBj/MarkSystems output) — hand off to the `rtk-operations` skill. To baseline a machine before rollout or diagnose "no savings", run `rtk-audit` (`rtk init --show`, `rtk gain`, `rtk discover`, `rtk session`, `rtk verify`, and optional `rtk learn`). For live mid-session troubleshooting, run the `/diagnose` command (Claude Code); to dry-run "does RTK filter this command?", run `/test-routing <cmd>`.

## Hook mechanics (so you can explain it to a skeptical developer)

- **Rewrite vs suggest.** The hook `rtk init` installs is a **rewrite** hook: it transparently replaces the command with `rtk <cmd>` before execution, so the agent sees filtered output without changing how it runs commands. RTK can also run in **suggest** mode (a nudge that tells the agent "RTK available: `rtk …`" without rewriting) — lower friction during gradual rollout, but less savings. ECI default is rewrite.
- **`rtk init` vs committed `.claude/hooks/`.** On 0.43.x the hook is a **native binary** registered in the agent's settings (`~/.claude/settings.json` for Claude Code, `~/.cursor/hooks.json` for Cursor) — not a `.claude/hooks/rtk-rewrite.sh` script file. `rtk init --show` is the source of truth for whether it is registered (`[ok]` markers). Do not hand-edit hook config; re-run `rtk init` instead.
- **Audit trail.** Set `RTK_HOOK_AUDIT=1` to log what the hook rewrote — use during security review or when diagnosing "did RTK touch this command?". The log is local only.
- **Never blocks.** On any internal error the hook passes through to the raw command (fail-open) — RTK cannot break a command by design.
- **Bypass is structural, not a bug.** Built-in Read/Grep/Glob and MCP tool outputs never enter the shell, so the hook cannot see them. That is why skills/rules tell the agent to prefer shell `rg`/`cat`/`find` or `rtk read`/`rtk grep`/`rtk find` for those paths.
